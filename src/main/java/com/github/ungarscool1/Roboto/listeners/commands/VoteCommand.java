	...
	public void onMessageCreate(MessageCreateEvent event) {
		...
		if (message.getContent().contains("!vote")) {
			...
			if (!votes.containsKey(message.getAuthor().asUser().get())) {
				...
				try {
					msg = event.getChannel().sendMessage(new EmbedBuilder().setTitle("Création de vote").setDescription("Quelle est le nom ?")).get();
				} catch (Exception e) {
					e.printStackTrace();
				}
				...
				if (res.equals("description")) {
					try {
						msg = event.getChannel().sendMessage(new EmbedBuilder().setTitle("Création de vote").setDescription("Quelle est la description ?")).get();
					} catch (Exception e) {
						e.printStackTrace();
					}
				} else if (res.equals("multi")) {
					try {
						msg = event.getChannel().sendMessage(new EmbedBuilder().setTitle("Création de vote").setDescription("Est-ce qu'il y a plusieurs réponses ?")).get();
					} catch (Exception e) {
						e.printStackTrace();
					}
					...
					try {
						event.getChannel().sendMessage(new EmbedBuilder().setTitle(vote.getName()).setDescription(vote.getDescription()).setColor(new Color(107, 135, 232)).setFooter("Vote créé par " + user.getDisplayName(message.getServer().get()))).get().addReactions("👍","👎");
					} catch (Exception e) {
					...
					try {
						msg = event.getChannel().sendMessage(new EmbedBuilder().setTitle("Création de vote").setDescription("Combien il y a de réponses ? (entre 1 et 10)")).get();
					} catch (Exception e) {
						e.printStackTrace();
					}
				} else if(res.equals("option")) {
					try {
						msg = event.getChannel().sendMessage(new EmbedBuilder().setTitle("Création de vote").setDescription("Réponse ?")).get();
					} catch (Exception e) {
						e.printStackTrace();
					}
				} else if(res.equals("fin multi")) {
					Vote vote = votes.get(user);
					EmbedBuilder embed = new EmbedBuilder().setTitle(vote.getName()).setDescription(vote.getDescription()).setColor(new Color(107, 135, 232)).setFooter("Vote créé par " + user.getDisplayName(message.getServer().get()));
					for(int i = 0; i < vote.getOptions().length; i++) {
						embed.addField("Réponse n°" + (i + 1), vote.getOptions()[i]);
					}
					try {
